...
public class sd_magicVectorThruster implements EveryFrameWeaponEffectPlugin {
    ...
    //Step 1: Figure out how to render fast engine thrust at system slots
    //Step 2: Use the thrust vectoring code to vector it
    //Step 3: Check the underlying engine slot, so we can check if we're flamed out
...
//                );
    ...
    private void thrust(WeaponAPI weapon, float angle, float thrust, float smooth) {
        ...
        //fer_EnginePlugin.addFlare(engine, flare, shipAPI);
        ...
        float width = length * size.x / 3 + size.x / 3;
        float height = length * size.y + (float) Math.random() * 2 + 2;
        ...
        //sprite.setColor(new Color(185,160,85,255));

        //sprite.setColor(new Color(255,125,25,255));
        sprite.setColor(new Color(250,235,215,200));
    }
