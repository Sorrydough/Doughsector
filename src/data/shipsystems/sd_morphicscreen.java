...
public class sd_morphicscreen extends BaseShipSystemScript { //TODO: DEBUG THIS
	public void apply(MutableShipStatsAPI stats, String id, State state, float effectLevel) {
		...
		stats.getShieldUpkeepMult().modifyMult(id, 3 * effectLevel);
		...
		float amountAbleToTransfer = ship.getArmorGrid().getArmorValue((int)cellToSubtract.x, (int)cellToSubtract.y) - ship.getArmorGrid().getMaxArmorInCell();
		float amountToTransfer = Math.max(amountNeededToTransfer, amountAbleToTransfer);
	...
	private float getAverageArmorPerCell(ArmorGridAPI grid) {
		float armor = 0f;
		for (int ix = 0; ix < grid.getGrid().length; ix++) {
			for (int iy = 0; iy < grid.getGrid()[0].length; iy++) {
	...
	public StatusData getStatusData(int index, State state, float effectLevel) {
		...
		if (index == 3) {
			return new StatusData("Shield upkeep tripled", true);
		}
		return null;
	}
